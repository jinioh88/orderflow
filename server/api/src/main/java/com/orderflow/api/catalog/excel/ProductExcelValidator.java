...
public class ProductExcelValidator {
    ...
    /**
     * @param existingCodes         테넌트에 이미 존재하는 품목코드 (수정 대상 판별)
     * @param existingBarcodeOwners 테넌트에 이미 존재하는 바코드 → 소유 상품의 품목코드
     */
        ...
        for (ProductExcelRow row : rows) {
            ...
            if (errors.size() == before) {
                commands.add(new ProductUpsertCommand(row.rowNum(),
                        !existingCodes.contains(row.productCode()), row.productCode(), row.name(),
    ...
    private void validateDuplicates(ProductExcelRow row,
        ...
        if (!row.productCode().isBlank()) {
            Integer firstRow = seenCodes.putIfAbsent(row.productCode(), row.rowNum());
            if (firstRow != null) {
        ...
        if (!row.barcode().isBlank()) {
            Integer firstRow = seenBarcodes.putIfAbsent(row.barcode(), row.rowNum());
            if (firstRow != null) {
                errors.add(new ExcelRowError(row.rowNum(), "barcode",
                        "파일 내 바코드가 중복됩니다 (%d행과 동일).".formatted(firstRow)));
            }
            String owner = existingBarcodeOwners.get(row.barcode());
            if (owner != null && !owner.equals(row.productCode())) {
